package it.govpay.model;

// TODO: Auto-generated Javadoc
/**
 * The Class Costanti.
 */
public class Costanti {
	
	/** The Constant logoPagoPa. */
	public static final String logoPagoPa = "";
	
	/**  COSTANTI PDF ESTRATTO CONTO. */
	
	public static final String TITOLO_REPORT = "Elenco dei Pagamenti PagoPA";
	
	/** The Constant TITOLO_PERIODO. */
	public static final String TITOLO_PERIODO = "{0} - {1}";
	
	/** The Constant PATTERN_DATA_PERIODO_REPORT. */
	public static final String PATTERN_DATA_PERIODO_REPORT = "dd/MM/yyyy";
	
	/** The Constant TITOLO_RIEPILOGO. */
	public static final String TITOLO_RIEPILOGO = "Informazioni Generali";
	
	/** The Constant LABEL_IBAN_ACCREDITO. */
	public static final String LABEL_IBAN_ACCREDITO = "Iban di Accredito";
	
	/** The Constant LABEL_PAESE. */
	public static final String LABEL_PAESE = "Paese";
	
	/** The Constant LABEL_EUR. */
	public static final String LABEL_EUR = "Eur";
	
	/** The Constant LABEL_CIN. */
	public static final String LABEL_CIN = "Cin";
	
	/** The Constant LABEL_ABI. */
	public static final String LABEL_ABI = "Abi";
	
	/** The Constant LABEL_CAB. */
	public static final String LABEL_CAB = "Cab";
	
	/** The Constant LABEL_NUMERO_CONTO. */
	public static final String LABEL_NUMERO_CONTO = "N. Conto";
	
	/** The Constant LABEL_NUMERO_PAGAMENTI. */
	public static final String LABEL_NUMERO_PAGAMENTI = "Numero Pagamenti";
	
	/** The Constant LABEL_IMPORTO_TOTALE. */
	public static final String LABEL_IMPORTO_TOTALE = "Importo Totale";
	
	/** The Constant LABEL_EURO. */
	public static final String LABEL_EURO = "€";
	
	/** The Constant LABEL_ID_FLUSSO_RENDICONTAZIONE. */
	public static final String LABEL_ID_FLUSSO_RENDICONTAZIONE = "Id Flusso Rendicontazione";
	
	/** The Constant LABEL_DETTAGLIO_PAGAMENTI. */
	public static final String LABEL_DETTAGLIO_PAGAMENTI = "Dettaglio Pagamenti";
	
	/** The Constant LABEL_IUV. */
	public static final String LABEL_IUV = "Iuv";
	
	/** The Constant LABEL_ULTERIORI_INFORMAZIONI. */
	public static final String LABEL_ULTERIORI_INFORMAZIONI = "Ulteriori Informazioni";
	
	/** The Constant LABEL_IMPORTO_PAGATO. */
	public static final String LABEL_IMPORTO_PAGATO = "Importo Pagato";
	
	/** The Constant LABEL_ID_RIVERSAMENTO. */
	public static final String LABEL_ID_RIVERSAMENTO = "Id Riversamento";
	
	/** The Constant LABEL_DATA_PAGAMENTO. */
	public static final String LABEL_DATA_PAGAMENTO = "Data Pagamento";
	
	/** The Constant LABEL_CODICE_VERSAMENTO_ENTE. */
	public static final String LABEL_CODICE_VERSAMENTO_ENTE = "Cod Versamento Ente";
	
	/** The Constant LABEL_BIC_RIVERSAMENTO. */
	public static final String LABEL_BIC_RIVERSAMENTO = "Bic Riversamento";
	
	/** The Constant LABEL_ID_REGOLAMENTO. */
	public static final String LABEL_ID_REGOLAMENTO = "Id Regolamento";
	
	/** The Constant LABEL_DUE_PUNTI. */
	public static final String LABEL_DUE_PUNTI = ":";
	
	/** The Constant LABEL_P_IVA. */
	public static final String LABEL_P_IVA = "P.IVA";
	
	/** The Constant PATTERN_NOME_DUE_PUNTI_VALORE. */
	public static final String PATTERN_NOME_DUE_PUNTI_VALORE = "{0}: {1}";
	
	/** The Constant LABEL_COMMISSIONI_PSP. */
	public static final String LABEL_COMMISSIONI_PSP = "Commissioni PSP";
	
	
	
	/**  Nomi dei campi del datasource  TABELLA ESTRATTO CONTO. */
	
	public static final String IDENTIFICATIVO_VERSAMENTO_COL = "codVersamentoEnte";
	
	/** The Constant IMPORTO_PAGATO_COL. */
	public static final String IMPORTO_PAGATO_COL = "importoPagato";
	
	/** The Constant DATA_PAGAMENTO_COL. */
	public static final String DATA_PAGAMENTO_COL = "dataPagamento";
	
	/** The Constant CODICE_RIVERSAMENTO_COL. */
	public static final String CODICE_RIVERSAMENTO_COL = "codRiversamento";
	
	/** The Constant CODICE_RENDICONTAZIONE_COL. */
	public static final String CODICE_RENDICONTAZIONE_COL = "codRendicontazione";
	
	/** The Constant NOTE_COL. */
	public static final String NOTE_COL = "note";
	
	/** The Constant BIC_RIVERSAMENTO_COL. */
	public static final String BIC_RIVERSAMENTO_COL = "bicRiversamento";
	
	/** The Constant ID_REGOLAMENTO_COL. */
	public static final String ID_REGOLAMENTO_COL = "idRegolamento";
	
	/** The Constant IBAN_ACCREDITO_COL. */
	public static final String IBAN_ACCREDITO_COL = "ibanAccredito";
	
	/** The Constant IUV_COL. */
	public static final String IUV_COL = "iuv";
	
	/** The Constant COL_UNO. */
	public static final String COL_UNO = "colUno";
	
	/** The Constant PAGAMENTI_SENZA_RPT_KEY. */
	public static final String PAGAMENTI_SENZA_RPT_KEY = "PagamentiSenzaRpt";
	
	/** The Constant LABEL_PAGAMENTI_SENZA_RPT. */
	public static final String LABEL_PAGAMENTI_SENZA_RPT = "Pagamenti Senza Rpt";
	
	/** The Constant MARCA_DA_BOLLO_KEY. */
	public static final String MARCA_DA_BOLLO_KEY = "MarcaDaBolloTelematica";
	
	/** The Constant LABEL_MARCA_DA_BOLLO_TELEMATICA. */
	public static final String LABEL_MARCA_DA_BOLLO_TELEMATICA = "Marca da Bollo Telematica";
	
	/**  LABEL PDF RT. */
	
	public static final String LABEL_RICEVUTA_PAGAMENTO_UPPER_CASE = "RICEVUTA DI PAGAMENTO N°";
	
	/** The Constant LABEL_CF_PIVA. */
	public static final String LABEL_CF_PIVA = "Codice Fiscale/P.IVA";
	
	/** The Constant LABEL_RICEVUTA_PAGAMENTO. */
	public static final String LABEL_RICEVUTA_PAGAMENTO = "RICEVUTA DI PAGAMENTO";
	
	/** The Constant LABEL_DETTAGLIO_PAGAMENTO. */
	public static final String LABEL_DETTAGLIO_PAGAMENTO = "DETTAGLIO DEL PAGAMENTO";
	
	/** The Constant LABEL_CF_PIVA_DEBITORE. */
	public static final String LABEL_CF_PIVA_DEBITORE = "Codice Fiscale/P.IVA del debitore";
//	public static final String LABEL_DATA_PAGAMENTO = "Data di pagamento";
/** The Constant LABEL_IMPORTO_DOVUTO. */
//	public static final String LABEL_IUV = "IUV";
	public static final String LABEL_IMPORTO_DOVUTO = "Importo Dovuto";
	
	/** The Constant LABEL_IUR. */
	public static final String LABEL_IUR = "Identificativo della riscossione";
	
	/** The Constant LABEL_ISTITUTO_ATTESTANTE. */
	public static final String LABEL_ISTITUTO_ATTESTANTE = "Istituto attestante";
	
	/** The Constant LABEL_CAUSALE. */
	public static final String LABEL_CAUSALE = "Causale";
	
	/** The Constant LABEL_CAUSALE_BONIFICIO. */
	public static final String LABEL_CAUSALE_BONIFICIO = "Causale Bonifico";
	
	/** The Constant LABEL_CAUSALE_VERSAMENTO. */
	public static final String LABEL_CAUSALE_VERSAMENTO = "Causale Versamento";
	
	/** The Constant LABEL_DESCRIZIONE_CAUSALE. */
	public static final String LABEL_DESCRIZIONE_CAUSALE = "Descrizione Causale";
	
	/** The Constant LABEL_TERMINE_PAGAMENTO. */
	public static final String LABEL_TERMINE_PAGAMENTO = "Termine di pagamento";
	
	/** The Constant LABEL_IN_CORSO. */
	public static final String LABEL_IN_CORSO = "In Corso"; 
	
	/** The Constant LABEL_STATO_VERSAMENTO_ANNULLATO. */
	public static final String LABEL_STATO_VERSAMENTO_ANNULLATO = "Annullato";
	
	/** The Constant LABEL_STATO_VERSAMENTO_ANOMALO. */
	public static final String LABEL_STATO_VERSAMENTO_ANOMALO = "Pagato con anomalie";
	
	/** The Constant LABEL_STATO_VERSAMENTO_ESEGUITO. */
	public static final String LABEL_STATO_VERSAMENTO_ESEGUITO = "Pagato";
	
	/** The Constant LABEL_STATO_VERSAMENTO_ESEGUITO_SENZA_RPT. */
	public static final String LABEL_STATO_VERSAMENTO_ESEGUITO_SENZA_RPT = "Pagato";
	
	/** The Constant LABEL_STATO_VERSAMENTO_NON_ESEGUITO. */
	public static final String LABEL_STATO_VERSAMENTO_NON_ESEGUITO = "In attesa di pagamento";
	
	/** The Constant LABEL_STATO_VERSAMENTO_PARZIALMENTE_ESEGUITO. */
	public static final String LABEL_STATO_VERSAMENTO_PARZIALMENTE_ESEGUITO = "Parzialmente Eseguito";
	
	/** The Constant LABEL_STATO_VERSAMENTO_SCADUTO. */
	public static final String LABEL_STATO_VERSAMENTO_SCADUTO = "Scaduto";
	
	/** The Constant LABEL_STATO_VERSAMENTO_IN_CORSO. */
	public static final String LABEL_STATO_VERSAMENTO_IN_CORSO = "In attesa di ricevuta";
	
	/** The Constant COLORE_LABEL_STATO_VERSAMENTO_ROSSO. */
	public static final String COLORE_LABEL_STATO_VERSAMENTO_ROSSO = "stato-error";
	
	/** The Constant COLORE_LABEL_STATO_VERSAMENTO_GIALLO. */
	public static final String COLORE_LABEL_STATO_VERSAMENTO_GIALLO = "stato-warning";
	
	/** The Constant COLORE_LABEL_STATO_VERSAMENTO_VERDE. */
	public static final String COLORE_LABEL_STATO_VERSAMENTO_VERDE = "stato-info";
	
	/** The Constant LABEL_PAGAMENTO_PARZIALMENTE_ESEGUITO. */
	public static final String LABEL_PAGAMENTO_PARZIALMENTE_ESEGUITO = "Eseguito Parzialmente";
	
	/** The Constant LABEL_PAGAMENTO_NON_ESEGUITO. */
	public static final String LABEL_PAGAMENTO_NON_ESEGUITO = "Non Eseguito";
	
	/** The Constant LABEL_PAGAMENTO_ESEGUITO. */
	public static final String LABEL_PAGAMENTO_ESEGUITO = "Eseguito";
	
	/** The Constant LABEL_DECORRENZA_TERMINI_PARZIALE. */
	public static final String LABEL_DECORRENZA_TERMINI_PARZIALE = "Decorrenza Termini Parziale";
	
	/** The Constant LABEL_DECORRENZA_TERMINI. */
	public static final String LABEL_DECORRENZA_TERMINI = "Decorrenza Parziale";
	
	/** The Constant LABEL_TIPO_VERSAMENTO_CP. */
	public static final String LABEL_TIPO_VERSAMENTO_CP = "Carta di Pagamento";
	
	/** The Constant LABEL_TIPO_VERSAMENTO_BBT. */
	public static final String LABEL_TIPO_VERSAMENTO_BBT = "Bonifico Bancario";
	
	/** The Constant LABEL_TIPO_VERSAMENTO_BP. */
	public static final String LABEL_TIPO_VERSAMENTO_BP = "Bollettino Postale";
	
	/** The Constant LABEL_TIPO_VERSAMENTO_AD. */
	public static final String LABEL_TIPO_VERSAMENTO_AD = "Addebito diretto";
	
	/** The Constant LABEL_TIPO_VERSAMENTO_OBEP. */
	public static final String LABEL_TIPO_VERSAMENTO_OBEP = "On-line Banking e-payment";
	
	/** The Constant LABEL_TIPO_VERSAMENTO_PO. */
	public static final String LABEL_TIPO_VERSAMENTO_PO = "Pagamento attivato presso PSP";
	
	
	/** The Constant LABEL_DATA_REVOCA. */
	public static final String LABEL_DATA_REVOCA = "Data Revoca";
	
	/** The Constant LABEL_ESITO_REVOCA. */
	public static final String LABEL_ESITO_REVOCA = "RICEVUTA DI REVOCA";
	
	/** The Constant LABEL_IMPORTO_REVOCA. */
	public static final String LABEL_IMPORTO_REVOCA = "Importo Revocato";
	
	/** The Constant LABEL_CCP. */
	public static final String LABEL_CCP = "Ccp";
	
	/** The Constant LABEL_ID_RISCOSSIONE. */
	public static final String LABEL_ID_RISCOSSIONE = "Id Riscossione";
	
	/** The Constant LABEL_PSP. */
	public static final String LABEL_PSP = "Psp";
	
	/** The Constant LABEL_ATTESTANTE. */
	public static final String LABEL_ATTESTANTE = "Istituto di pagamento utilizzato";
	
	/** The Constant LABEL_PAGATORE. */
	public static final String LABEL_PAGATORE = "Pagatore";
	
	/** The Constant LABEL_VERSANTE. */
	public static final String LABEL_VERSANTE = "Versante";
	
	/** The Constant LABEL_DEBITORE. */
	public static final String LABEL_DEBITORE = "Debitore";
	
	
	/**  LABEL AVVISO PAGAMENTO *. */
	public static final String LABEL_RIFERIMENTI = "Riferimenti";
	
	/** The Constant LABEL_COMUNICAZIONI. */
	public static final String LABEL_COMUNICAZIONI = "Comunicazioni";
	
	/** The Constant LABEL_DATA_SCADENZA. */
	public static final String LABEL_DATA_SCADENZA = "Data Scadenza";
	
	/** The Constant LABEL_IMPORTO. */
	public static final String LABEL_IMPORTO = "Importo";
	
	/** The Constant LABEL_IMPORTO_DA_PAGARE. */
	public static final String LABEL_IMPORTO_DA_PAGARE = "Importo da Pagare";
	
	/** The Constant LABEL_SCADENZA_DEL_PAGAMENTO. */
	public static final String LABEL_SCADENZA_DEL_PAGAMENTO = "Scadenze di Pagamento";
	
	/** The Constant LABEL_CODICE_FISCALE_CREDITORE. */
	public static final String LABEL_CODICE_FISCALE_CREDITORE = "Codice Fiscale/P.IVA Ente Creditore";
	
	/** The Constant LABEL_CODICE_AVVISO_PAGAMENTO. */
	public static final String LABEL_CODICE_AVVISO_PAGAMENTO = "Avviso di Pagamento N°";
	
	/** The Constant LABEL_CODICE_AVVISO. */
	public static final String LABEL_CODICE_AVVISO = "Codice Avviso";
	
	/** The Constant LABEL_CODICE_VERSAMENTO. */
	public static final String LABEL_CODICE_VERSAMENTO = "Codice Versamento";
	
	/** The Constant LABEL_CODICE_AVVISO_PAGAMENTO_UPPER_CASE. */
	public static final String LABEL_CODICE_AVVISO_PAGAMENTO_UPPER_CASE = "AVVISO DI PAGAMENTO N°";
	
	/** The Constant LABEL_INTESTATO_A. */
	public static final String LABEL_INTESTATO_A = "Intestato a:";
	
	/** The Constant LABEL_ESTREMI_DI_PAGAMENTO. */
	public static final String LABEL_ESTREMI_DI_PAGAMENTO = "Estremi di Pagamento";
	
	/**  LABEL  INCASSI *. */
	
	public static final String TITOLO_REPORT_INCASSO = "Report Incasso";
	
	/** The Constant LABEL_CREDITORE. */
	public static final String LABEL_CREDITORE = "Creditore";
	
	/** The Constant LABEL_IUV2. */
	public static final String LABEL_IUV2 = "IUV";
	
	/** The Constant LABEL_IUR2. */
	public static final String LABEL_IUR2 = "IUR";
	
	/** The Constant LABEL_TRN. */
	public static final String LABEL_TRN = "TRN";
	
	/** The Constant LABEL_DATA_CONTABILE. */
	public static final String LABEL_DATA_CONTABILE = "Data Contabile";
	
	/** The Constant LABEL_DATA_VALUTA. */
	public static final String LABEL_DATA_VALUTA = "Data Valuta";
	
	/** The Constant LABEL_DATA_INCASSO. */
	public static final String LABEL_DATA_INCASSO = "Data Incasso";
	
	/** The Constant LABEL_APPLICAZIONE. */
	public static final String LABEL_APPLICAZIONE = "Applicazione";
	
	/** The Constant LABEL_DISPOSITIVO. */
	public static final String LABEL_DISPOSITIVO = "Dispositivo";

	/**  COLONNE INCASSI *. */
	
	public static final String CREDITORE_COL = "creditore";
	
	/** The Constant IUR_COL. */
	public static final String IUR_COL = "iur";
	
	/** The Constant IMPORTO_COL. */
	public static final String IMPORTO_COL = "importo";
	
}
